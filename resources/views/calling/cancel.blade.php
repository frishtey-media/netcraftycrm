...
@section('content')
    ...
    <div class="table-responsive d-none d-md-block">
        <div class="client-scroll mb-3">
            ...
            <!-- ALL -->
            <a href="{{ route('calling.cancel') }}" class="client-chip {{ request('client_id') ? '' : 'active' }}">
                All
            </a>

            <!-- CLIENT LOOP -->
            @foreach ($clients as $row)
                <a href="{{ route('calling.cancel', ['client_id' => $row->client_id]) }}"
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @elseif($order->status == 'same_order')
                                <span class="badge bg-warning text-dark">Same Order</span>
                            @elseif($order->status == 'cancel')
                                <span class="badge bg-warning text-dark">Cancel Order</span>
                            @else
    ...
    <div class="d-block d-md-none">
        <div class="client-scroll mb-3">
            ...
            <!-- ALL -->
            <a href="{{ route('calling.same_order') }}" class="client-chip {{ request('client_id') ? '' : 'active' }}">
                All
            </a>

            <!-- CLIENT LOOP -->
            @foreach ($clients as $row)
                <a href="{{ route('calling.same_order', ['client_id' => $row->client_id]) }}"
                    class="client-chip {{ request('client_id') == $row->client_id ? 'active' : '' }}">
